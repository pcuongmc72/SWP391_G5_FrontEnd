const fs = require('fs');
const path = 'src/pages/Lecturer/MaterialsDashboard.jsx';
...
content = content.replace(/editMaterialForm\.inputType/g, "editMaterialForm.type");
content = content.replace(/newMaterialForm\.inputType/g, "newMaterialForm.type");

// 2. Update dropdown options in Add and Edit Modals
const old_select = `<select className={styles.select} value={newMaterialForm.type}
...
const new_select = `<select className={styles.select} value={newMaterialForm.type}
                    onChange={(e) => setNewMaterialForm({ ...newMaterialForm, type: e.target.value })}>
...
const old_edit_select = `<select className={styles.select} value={editMaterialForm.type} onChange={(e) => setEditMaterialForm({ ...editMaterialForm, type: e.target.value })}>
                        <option value="video">Video bài giảng (quay trước)</option>
                        <option value="pdf">Tài liệu PDF</option>
                        <option value="document">Giáo trình doc</option>
                        <option value="quiz">Trắc nghiệm</option>
                      </select>`;
const new_edit_select = `<select className={styles.select} value={editMaterialForm.type} onChange={(e) => setEditMaterialForm({ ...editMaterialForm, type: e.target.value })}>
                        <option value="image">Hình ảnh (Tải lên)</option>
                        <option value="video">Link Video</option>
                      </select>`;
...
const radio_btns = `<div style={{ display: 'flex', gap: 16, marginBottom: 12, borderBottom: '1px solid #e2e8f0', paddingBottom: 12 }}>
...
const radio_btns_edit = `<div style={{ display: 'flex', gap: 16, marginBottom: 12, borderBottom: '1px solid #e2e8f0', paddingBottom: 12 }}>
...
content = content.replace(/{newMaterialForm\.type === 'file' \?/g, "{newMaterialForm.type === 'image' ?");
content = content.replace(/{editMaterialForm\.type === 'file' \?/g, "{editMaterialForm.type === 'image' ?");
...
const old_detect = `  const detectFileType = (file) => {
...
const new_detect = `  const detectFileType = (file) => {
    return 'image';
  };`;
content = content.replace(old_detect, new_detect);

// 8. Update renderFileIcon
const old_icon = `      case 'pdf': return <FileText size={32} color="#ef4444" />;
      case 'document': return <FileSpreadsheet size={32} color="#10b981" />;
      case 'quiz': return <CheckSquare size={32} color="#f59e0b" />;`;
const new_icon = `      case 'image': return <ImageIcon size={32} color="#10b981" />;`;
content = content.replace(old_icon, new_icon);

// Add ImageIcon to imports if not there
if (!content.includes('ImageIcon')) {
    content = content.replace('FileSpreadsheet,', 'FileSpreadsheet, ImageIcon,');
}

fs.writeFileSync(path, content, 'utf-8');
console.log("Updated via Node.js script.");
